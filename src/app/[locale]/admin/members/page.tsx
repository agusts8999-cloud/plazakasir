...
import { UserPlus, Search, Building2, Calendar, Mail, Phone, Users } from "lucide-react";
...
export default async function AdminMembersPage({ 
  ...
  const activeTab = searchParamsResolved.tab || "all";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-full">
        <div className="bg-muted/50 p-1 rounded-lg h-11 border border-border mb-6 inline-flex w-full items-center">
          <Link 
            href="/admin/members?tab=all" 
            className={cn(
              "flex-1 flex items-center justify-center rounded-md h-9 px-4 font-semibold text-xs gap-2 transition-all",
          ...
          <Link 
            href="/admin/members?tab=admin" 
            className={cn(
              "flex-1 flex items-center justify-center rounded-md h-9 px-4 font-semibold text-xs gap-2 transition-all",
              activeTab === "admin" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
            )}
          >
            Admin <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-[10px]">{getTabCount("ADMIN")}</Badge>
          </Link>
          <Link 
            href="/admin/members?tab=user" 
            className={cn(
              "flex-1 flex items-center justify-center rounded-md h-9 px-4 font-semibold text-xs gap-2 transition-all",
              activeTab === "user" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
            )}
          >
            Staff <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-[10px]">{getTabCount("USER")}</Badge>
          </Link>
          <Link 
            href="/admin/members?tab=customer" 
            className={cn(
              "flex-1 flex items-center justify-center rounded-md h-9 px-4 font-semibold text-xs gap-2 transition-all",
              activeTab === "customer" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
            )}
          >
