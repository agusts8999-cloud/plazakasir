...
export async function updateSetting(formData: FormData) {
  ...
  try {
    ...
    // Handle Locale
    const locale = formData.get("_locale") as string || "id";

    // Get all keys from formData (excluding internal Next.js ones and the file object)
    const keys = Array.from(formData.keys()).filter(k => 
      !k.startsWith("$") && 
      k !== "site_logo_file" && 
      k !== "_locale" && 
      k !== "site_logo"
    ...
    const updates = keys.map(key => {
      let val = formData.get(key) as string;
      
      // Determine if this key should be localized
      // For now, we localize almost everything except specifically excluded keys
      const excludedFromLocalization = [
        "midtrans_client_key", 
        "midtrans_server_key", 
      ...
      const finalKey = (locale !== 'id' && !excludedFromLocalization.includes(key)) 
        ? `${locale}_${key}` 
        : key;

      return { key: finalKey, value: val };
    });

    // Add site_logo (usually global, not localized)
    updates.push({ key: "site_logo", value: logoUrl });
